...
const CarouselCard: React.FC<CarouselCardProps> = ({
  ...
  return (
    <motion.div
      className="drop-shadow-lg grid bg-red-400 rounded-3xl hover:cursor-pointer "
    ...
    >
      {showHoverInfo && (
        <div className="opacity-75 absolute w-full h-full bg-black rounded-3xl hover:brightness-[25%] cursor-pointer"></div>
      )}
      {showHoverInfo && (
        <div className="absolute z-[2] flex justify-center p-1 items-center text-center h-full w-full text-[10px]">
      ...
      <div className="self-start justify-self-end flex text-3xl z-[2]">
        ...
        >
          <GrGallery className="mr-2 z-[10]" />
        </Link>

        <a href={github} className={github ? "visible z-[100]" : "hidden"}>
          <FaGithub className="mr-2 z-[100]" />
        </a>
        <a href={website} className={website ? "visible z-[100]" : "hidden"}>
          <TbWorld />
        </a>
      </div>
      <div className="justify-self-start self-end font-lobster text-3xl">
        ...
        {categories && (
          <div
            className={`${categoriesClass} flex flex-wrap  h-1/2 bg-gray-200`}
          >
